...
import { getViteTemplate } from './templates/vite.js';
import { getNextTemplate } from './templates/next.js';
...
async function createViteProject(projectPath: string, config: ProjectConfig): Promise<void> {
  const template = getViteTemplate(config);
  
  await fs.writeJson(path.join(projectPath, 'package.json'), template.packageJson, { spaces: 2 });
  
  await fs.writeFile(path.join(projectPath, 'index.html'), template.html);
  await fs.writeFile(path.join(projectPath, 'vite.config.ts'), template.viteConfig);
  await fs.writeFile(path.join(projectPath, 'tsconfig.json'), template.tsconfig);
  await fs.writeFile(path.join(projectPath, 'tsconfig.app.json'), template.tsconfigApp);
  await fs.writeFile(path.join(projectPath, 'tsconfig.node.json'), template.tsconfigNode);
  await fs.writeFile(path.join(projectPath, 'tailwind.config.js'), template.tailwindConfig);
  
  await fs.ensureDir(path.join(projectPath, 'src'));
  await fs.writeFile(path.join(projectPath, 'src/main.tsx'), template.mainTsx);
  await fs.writeFile(path.join(projectPath, 'src/App.tsx'), template.appTsx);
  await fs.writeFile(path.join(projectPath, 'src/index.css'), template.indexCss);
  await fs.writeFile(path.join(projectPath, 'src/vite-env.d.ts'), template.viteEnvDts);
  
  await fs.ensureDir(path.join(projectPath, 'public'));
  await fs.writeFile(path.join(projectPath, 'public/vite.svg'), template.viteSvg);
  
  await fs.writeFile(path.join(projectPath, 'README.md'), template.readme);
}

async function createNextProject(projectPath: string, config: ProjectConfig): Promise<void> {
  const template = getNextTemplate(config);
  
  await fs.writeJson(path.join(projectPath, 'package.json'), template.packageJson, { spaces: 2 });
  
  await fs.writeFile(path.join(projectPath, 'next.config.js'), template.nextConfig);
  await fs.writeFile(path.join(projectPath, 'tsconfig.json'), template.tsconfig);
  await fs.writeFile(path.join(projectPath, 'tailwind.config.js'), template.tailwindConfig);
  
  await fs.ensureDir(path.join(projectPath, 'src'));
  await fs.ensureDir(path.join(projectPath, 'src/app'));
  await fs.writeFile(path.join(projectPath, 'src/app/layout.tsx'), template.layoutTsx);
  await fs.writeFile(path.join(projectPath, 'src/app/page.tsx'), template.pageTsx);
  await fs.writeFile(path.join(projectPath, 'src/app/globals.css'), template.globalsCss);
